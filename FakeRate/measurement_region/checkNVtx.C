{
  ...
  if (doMu) {
    //mm
    t_data->Draw("nvtx>>nvtx_data","HLT_Mu24*(abs(id)==13 && passes_SS_tight_v3 && HLT_Mu24>0 && tag_p4.pt()>30. && p4.pt()>25.)","goff");
    t_dy->Draw("nvtx>>nvtx_dy","0.040*scale1fb*(abs(id)==13 && passes_SS_tight_v3 && HLT_Mu24!=0 && tag_p4.pt()>30. && p4.pt()>25.)","goff");
  } else {
    //ee
    t_data->Draw("nvtx>>nvtx_data","abs(id)==11 && passes_SS_tight_v3 && HLT_Ele23_CaloIdM_TrackIdM_PFJet30==1 && tag_p4.pt()>30. && p4.pt()>25.","goff");
    t_dy->Draw("nvtx>>nvtx_dy","0.35*0.040*scale1fb*(abs(id)==11 && passes_SS_tight_v3 && HLT_Ele23_CaloIdM_TrackIdM_PFJet30!=0 && tag_p4.pt()>30. && p4.pt()>25.)","goff");
  }
  ...
  nvtx_ratio->Draw();
  TFitResultPtr fit = nvtx_ratio->Fit("pol1","S");
  ...
  for (int bin=1;bin<=nvtx_rw->GetNbinsX();++bin) {
    nvtx_rw->SetBinContent(bin, (fit->Parameter(0)+fit->Parameter(1)*nvtx_rw->GetBinCenter(bin))*nvtx_rw->GetBinContent(bin));
  }

...
int ScanChain( TChain* chain, TString fakeratefile, TString option = "", TString ptRegion = "HH", TChain* chain2 = NULL, int nEvents = -1){
  ...
  // NJA -- scan over MC chain to get prompt contamination in class2, then scan over data
  // probably a smarter way to do this
  float goodLep1Class2El = 0; // weighted evt counts for prompt class2 leptons failing ID
  float goodLep2Class2El = 0;
  float goodLep1Class2Mu = 0;
  float goodLep2Class2Mu = 0;
  float subtraction = 0;

  { // begin stupid block

  unsigned int nEventsTotal = 0;
  unsigned int nEventsChain = chain2->GetEntries();
  if( nEvents >= 0 ) nEventsChain = nEvents;
  TObjArray *listOfFiles = chain2->GetListOfFiles();
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
    ...
    for(unsigned int eventAG = 0; eventAG < nEventsTree; ++eventAG){
      ...
      if (lep1_passes_id && !lep2_passes_id){  
        ...
        // check for MC lepton failing ID but indeed having a prompt lepton
        if(isGoodLeg(2)) {
          if(abs(ss::lep1_id())==11) goodLep2Class2El += weight;
          else if(abs(ss::lep1_id())==13) goodLep2Class2Mu += weight;
        }
      }
      if (!lep1_passes_id && lep2_passes_id){  
        //Baseline selections
        ...
        // check for MC lepton failing ID but indeed having a prompt lepton
        if(isGoodLeg(1)) {
          if(abs(ss::lep1_id())==11) goodLep1Class2El += weight;
          else if(abs(ss::lep1_id())==13) goodLep1Class2Mu += weight;
        }
      }
    ...
    for(unsigned int eventAG = 0; eventAG < nEventsTree; ++eventAG){
      ...
      if(ss::hyp_class() == 2){ 
        ...
        if (lep1_passes_id && !lep2_passes_id){  
          ...
          if (abs(ss::lep2_id()) == 11){  
            if(ss::is_real_data()) subtraction = goodLep2Class2El;
            e2 = getFakeRate( rate_histo_e, lep2_pT, fabs(ss::lep2_p4().eta()), ss::ht(), false );
            hists[getHist("Npn_histo_sr_pred_el")]   ->Fill(sr, (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_br_pred_el")]   ->Fill(br, (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_HT_pred_el")]   ->Fill(ss::ht(), (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_MET_pred_el")]  ->Fill(ss::met(), (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_MTMIN_pred_el")]->Fill(mtmin, (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_L1PT_pred_el")] ->Fill(coneCorr ? lep1_pT : ss::lep1_p4().pt(), (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_L2PT_pred_el")] ->Fill(coneCorr ? lep2_pT : ss::lep2_p4().pt(), (e2/(1-e2))*weight-subtraction);
            if (sr>=0) Npn_histo_sr_err2_pred_el[sr]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_br_err2_pred_el[br]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_HT_err2_pred_el[hists[getHist("Npn_histo_HT_pred_el")]->FindBin(ss::ht())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_MET_err2_pred_el[hists[getHist("Npn_histo_MET_pred_el")]->FindBin(ss::met())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_MTMIN_err2_pred_el[hists[getHist("Npn_histo_MTMIN_pred_el")]->FindBin(mtmin)-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_L1PT_err2_pred_el[hists[getHist("Npn_histo_L1PT_pred_el")]->FindBin(coneCorr ? lep1_pT : ss::lep1_p4().pt())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_L2PT_err2_pred_el[hists[getHist("Npn_histo_L2PT_pred_el")]->FindBin(coneCorr ? lep2_pT : ss::lep2_p4().pt())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            // fill el abundance histos here w/ nbtags
            if(ss::lep2_motherID() == -1) hists[getHist("NBs_BR_histo_e")]->Fill(nbjets, weight-subtraction); //LOOSE!TIGHT, not LOOSE LIKE IN MEAS REGION
            if(ss::lep2_motherID() == -2 || ss::lep2_motherID() == 0) hists[getHist("NnotBs_BR_histo_e")]->Fill(nbjets, weight-subtraction);
            if(ss::lep2_motherID() == -1) Bs_e = Bs_e + weight-subtraction;
            if(ss::lep2_motherID() == -2 || ss::lep2_motherID() == 0) notBs_e = notBs_e + weight-subtraction;
          }
          else if (abs(ss::lep2_id()) == 13){ 
            if(ss::is_real_data()) subtraction = goodLep2Class2Mu;
            e2 = getFakeRate( rate_histo_mu, lep2_pT, fabs(ss::lep2_p4().eta()), ss::ht(), false );
            hists[getHist("Npn_histo_sr_pred_mu")]->Fill(sr, (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_br_pred_mu")]->Fill(br, (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_HT_pred_mu")]->Fill(ss::ht(), (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_MET_pred_mu")]->Fill(ss::met(), (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_MTMIN_pred_mu")]->Fill(mtmin, (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_L1PT_pred_mu")]->Fill(coneCorr ? lep1_pT : ss::lep1_p4().pt(), (e2/(1-e2))*weight-subtraction);
            hists[getHist("Npn_histo_L2PT_pred_mu")]->Fill(coneCorr ? lep2_pT : ss::lep2_p4().pt(), (e2/(1-e2))*weight-subtraction);
            if (sr>=0) Npn_histo_sr_err2_pred_mu[sr]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_br_err2_pred_mu[br]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_HT_err2_pred_mu[hists[getHist("Npn_histo_HT_pred_mu")]->FindBin(ss::ht())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_MET_err2_pred_mu[hists[getHist("Npn_histo_MET_pred_mu")]->FindBin(ss::met())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_MTMIN_err2_pred_mu[hists[getHist("Npn_histo_MTMIN_pred_mu")]->FindBin(mtmin)-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_L1PT_err2_pred_mu[hists[getHist("Npn_histo_L1PT_pred_mu")]->FindBin(coneCorr ? lep1_pT : ss::lep1_p4().pt())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            Npn_histo_L2PT_err2_pred_mu[hists[getHist("Npn_histo_L2PT_pred_mu")]->FindBin(coneCorr ? lep2_pT : ss::lep2_p4().pt())-1]->Fill(lep2_pT, fabs(ss::lep2_p4().eta()), weight-subtraction);
            if(ss::lep2_motherID() == -1) hists[getHist("NBs_BR_histo_mu")]->Fill(nbjets, weight-subtraction); //LOOSE!TIGHT, not LOOSE LIKE IN MEAS REGION
            if(ss::lep2_motherID() == -2 || ss::lep2_motherID() == 0) hists[getHist("NnotBs_BR_histo_mu")]->Fill(nbjets, weight-subtraction);
            if(ss::lep2_motherID() == -1) Bs_mu = Bs_mu + weight-subtraction;
            if(ss::lep2_motherID() == -2 || ss::lep2_motherID() == 0) notBs_mu = notBs_mu + weight-subtraction;
          }
          if(ss::is_real_data()) subtraction = goodLep2Class2Mu+goodLep2Class2El;
          Npn = Npn + (e2/(1-e2))*weight-subtraction;
          if (ss::lep2_motherID()==1) Npn_s = Npn_s + (e2/(1-e2))*weight-subtraction;
          hists[getHist("Npn_histo_sr_pred")]->Fill(sr, (e2/(1-e2))*weight-subtraction);
          hists[getHist("Npn_histo_br_pred")]->Fill(br, (e2/(1-e2))*weight-subtraction);
          hists[getHist("Npn_histo_HT_pred")]->Fill(ss::ht(), (e2/(1-e2))*weight-subtraction);
          hists[getHist("Npn_histo_MET_pred")]->Fill(ss::met(), (e2/(1-e2))*weight-subtraction);
          hists[getHist("Npn_histo_MTMIN_pred")]->Fill(mtmin, (e2/(1-e2))*weight-subtraction);
          hists[getHist("Npn_histo_L1PT_pred")]->Fill(coneCorr ? lep1_pT : ss::lep1_p4().pt(), (e2/(1-e2))*weight-subtraction);
          hists[getHist("Npn_histo_L2PT_pred")]->Fill(coneCorr ? lep2_pT : ss::lep2_p4().pt(), (e2/(1-e2))*weight-subtraction);
        }
        ...
        //2) Lep1 is loose!tight, lep2 is tight
          ...
          if( abs(ss::lep1_id()) == 11 ){	//if el, use el rate.  FILL WITH NONPROMPT			  
            if(ss::is_real_data()) subtraction = goodLep1Class2El;
            e1 = getFakeRate(rate_histo_e, lep1_pT, fabs(ss::lep1_p4().eta()), ss::ht(), false );
            hists[getHist("Npn_histo_sr_pred_el")]   ->Fill(sr, (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_br_pred_el")]   ->Fill(br, (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_HT_pred_el")]   ->Fill(ss::ht(), (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_MET_pred_el")]  ->Fill(ss::met(), (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_MTMIN_pred_el")]->Fill(mtmin, (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_L1PT_pred_el")] ->Fill(coneCorr ? lep1_pT : ss::lep1_p4().pt(), (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_L2PT_pred_el")] ->Fill(coneCorr ? lep2_pT : ss::lep2_p4().pt(), (e1/(1-e1))*weight-subtraction);

            if (sr>=0) Npn_histo_sr_err2_pred_el[sr]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_br_err2_pred_el[br]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_HT_err2_pred_el[hists[getHist("Npn_histo_HT_pred_el")]->FindBin(ss::ht())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_MET_err2_pred_el[hists[getHist("Npn_histo_MET_pred_el")]->FindBin(ss::met())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_MTMIN_err2_pred_el[hists[getHist("Npn_histo_MTMIN_pred_el")]->FindBin(mtmin)-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_L1PT_err2_pred_el[hists[getHist("Npn_histo_L1PT_pred_el")]->FindBin(coneCorr ? lep1_pT : ss::lep1_p4().pt())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_L2PT_err2_pred_el[hists[getHist("Npn_histo_L2PT_pred_el")]->FindBin(coneCorr ? lep2_pT : ss::lep2_p4().pt())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            // fill el abundance histos here w/ nbtags
            if(ss::lep1_motherID() == -1) hists[getHist("NBs_BR_histo_e")]->Fill(nbjets, weight-subtraction); //LOOSE!TIGHT, not LOOSE LIKE IN MEAS REGION
            if(ss::lep1_motherID() == -2 || ss::lep1_motherID() == 0) hists[getHist("NnotBs_BR_histo_e")]->Fill(nbjets, weight-subtraction);
            if(ss::lep1_motherID() == -1) Bs_e = Bs_e + weight-subtraction;
            if(ss::lep1_motherID() == -2 || ss::lep1_motherID() == 0) notBs_e = notBs_e + weight-subtraction;
          }
          else if( abs(ss::lep1_id()) == 13 ){ //if mu, use mu rate.  FILL WITH NONPROMPT				  
            if(ss::is_real_data()) subtraction = goodLep1Class2Mu;
            e1 = getFakeRate(rate_histo_mu, lep1_pT, fabs(ss::lep1_p4().eta()), ss::ht(), false );
            hists[getHist("Npn_histo_sr_pred_mu")]->Fill(sr, (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_br_pred_mu")]->Fill(br, (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_HT_pred_mu")]->Fill(ss::ht(), (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_MET_pred_mu")]->Fill(ss::met(), (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_MTMIN_pred_mu")]->Fill(mtmin, (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_L1PT_pred_mu")]->Fill(coneCorr ? lep1_pT : ss::lep1_p4().pt(), (e1/(1-e1))*weight-subtraction);
            hists[getHist("Npn_histo_L2PT_pred_mu")]->Fill(coneCorr ? lep2_pT : ss::lep2_p4().pt(), (e1/(1-e1))*weight-subtraction);

            if (sr>=0) Npn_histo_sr_err2_pred_mu[sr]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_br_err2_pred_mu[br]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_HT_err2_pred_mu[hists[getHist("Npn_histo_HT_pred_mu")]->FindBin(ss::ht())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_MET_err2_pred_mu[hists[getHist("Npn_histo_MET_pred_mu")]->FindBin(ss::met())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_MTMIN_err2_pred_mu[hists[getHist("Npn_histo_MTMIN_pred_mu")]->FindBin(mtmin)-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_L1PT_err2_pred_mu[hists[getHist("Npn_histo_L1PT_pred_mu")]->FindBin(coneCorr ? lep1_pT : ss::lep1_p4().pt())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            Npn_histo_L2PT_err2_pred_mu[hists[getHist("Npn_histo_L2PT_pred_mu")]->FindBin(coneCorr ? lep2_pT : ss::lep2_p4().pt())-1]->Fill(lep1_pT, fabs(ss::lep1_p4().eta()), weight-subtraction);
            // fill el abundance histos here w/ nbtags
            if(ss::lep1_motherID() == -1) hists[getHist("NBs_BR_histo_mu")]->Fill(nbjets, weight-subtraction); //LOOSE!TIGHT, not LOOSE LIKE IN MEAS REGION
            if(ss::lep1_motherID() == -2 || ss::lep1_motherID() == 0) hists[getHist("NnotBs_BR_histo_mu")]->Fill(nbjets, weight-subtraction);
            if(ss::lep1_motherID() == -1) Bs_mu = Bs_mu + weight-subtraction;
            if(ss::lep1_motherID() == -2 || ss::lep1_motherID() == 0) notBs_mu = notBs_mu + weight-subtraction;
          }
          if(ss::is_real_data()) subtraction = goodLep1Class2Mu+goodLep1Class2El;
          Npn = Npn + (e1/(1-e1))*weight-subtraction;
          if (ss::lep1_motherID()==1) Npn_s = Npn_s + (e1/(1-e1))*weight-subtraction;
          hists[getHist("Npn_histo_sr_pred")]->Fill(sr, (e1/(1-e1))*weight-subtraction);
          hists[getHist("Npn_histo_br_pred")]->Fill(br, (e1/(1-e1))*weight-subtraction);
          hists[getHist("Npn_histo_HT_pred")]->Fill(ss::ht(), (e1/(1-e1))*weight-subtraction);
          hists[getHist("Npn_histo_MET_pred")]->Fill(ss::met(), (e1/(1-e1))*weight-subtraction);
          hists[getHist("Npn_histo_MTMIN_pred")]->Fill(mtmin, (e1/(1-e1))*weight-subtraction);
          hists[getHist("Npn_histo_L1PT_pred")]->Fill(coneCorr ? lep1_pT : ss::lep1_p4().pt(), (e1/(1-e1))*weight-subtraction);
          hists[getHist("Npn_histo_L2PT_pred")]->Fill(coneCorr ? lep2_pT : ss::lep2_p4().pt(), (e1/(1-e1))*weight-subtraction);
        }
  ...
  // NJA
  // goodLep1Class2El goodLep1Class2Mu goodLep2Class2El goodLep2Class2Mu
  std::cout << " goodLep1Class2El: " << goodLep1Class2El << " goodLep1Class2Mu: " << goodLep1Class2Mu << " goodLep2Class2El: " << goodLep2Class2El << " goodLep2Class2Mu: " << goodLep2Class2Mu << std::endl;

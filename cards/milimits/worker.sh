...
echo "combine -M HybridNew --frequentist --testStat LHC -H ProfileLikelihood --seed -1 -T $Ntoys -m ${cut} ${pfx}${cut}${postfix}/card_sig_${lumi}ifb-hihi.txt  -v 0 --rAbsAcc=$AbsAcc --rRelAcc=$RelAcc"
...
for q in 0.025 0.16 0.5 0.84 0.975; do 
    echo "combine -M HybridNew --frequentist --testStat LHC -H ProfileLikelihood --seed -1 -T $Ntoys -m ${cut} ${pfx}${cut}${postfix}/card_sig_${lumi}ifb-hihi.txt  -v 0 --rAbsAcc=$AbsAcc --rRelAcc=$RelAcc --expectedFromGrid=${q}"

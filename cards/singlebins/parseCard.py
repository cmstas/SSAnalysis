def dump(fname):
    ...
    for channel,info in d_card.items():
        nsig = sum([info[k] for k in info if "fs_" in k and "data" not in k])
        ndata = sum([info[k] for k in info if "fs_" not in k and "data" in k])
        npred = sum([info[k] for k in info if "fs_" not in k and "data" not in k])

        print info
        signame = [k for k in info if "fs_" in k][0]

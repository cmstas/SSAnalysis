...
if not doSignificance:
    ...
    l1.SetHeader("pp #rightarrow #tilde{g}#tilde{g}, #tilde{g}#rightarrow t#bar{t}#tilde{#chi}^{0}_{1}        NLO+NLL cross section")
...
c1.SaveAs("t1tttt_scan_xsec_%s%s%s.pdf" % (lumi_str,postfix, "_sig" if doSignificance else ""))
...
os.system("web t1tttt_scan_xsec_%s%s%s.pdf" % (lumi_str,postfix,"_sig" if doSignificance else ""))

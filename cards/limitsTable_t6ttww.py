...
for msbo in range (minx,maxx,step):
    for mchi in range (miny,maxy,step):
        if os.path.isfile(("%s/fs_t6ttww_m%i_m%i_histos_hihi_%sifb.root" % (mydir,msbo,mchi,mylumi))) is False: continue
        #print "found sig = fs_t6ttww_m%i_m%i" % (msbo,mchi)
        sigs.append(("fs_t6ttww_m%i_m%i" % (msbo,mchi)))
...
if doSignificance:
    for sig in sigs:
        foundObs = False
        if not os.path.isfile(mydir+"/card_"+sig+"_"+mylumi+"ifb-all_significance.log"): continue
...
for sig in sigs:
    smod = sig.split("_")[1]
    msbo = int(sig.split("_")[2][1:])
    msbos.append(msbo)
    mchi = int(sig.split("_")[3][1:])
    mchis.append(mchi)

    if not doSignificance:
        v_xsec.append( limit_obs[count]*hxsec.GetBinContent(hxsec.FindBin(msbo)) )
        v_sobs.append( limit_obs[count] )
        v_som1.append( limit_obs[count]*(hxsec.GetBinContent(hxsec.FindBin(msbo))-hxsec.GetBinError(hxsec.FindBin(msbo)))/hxsec.GetBinContent(hxsec.FindBin(msbo)) )
        v_sop1.append( limit_obs[count]*(hxsec.GetBinContent(hxsec.FindBin(msbo))+hxsec.GetBinError(hxsec.FindBin(msbo)))/hxsec.GetBinContent(hxsec.FindBin(msbo)) )
        ...
        if verbose:
            print ("%-8s %4i %4i | %.1f | %.1f (-1s %.1f, +1s %.1f) | %.3f " % (smod , msbo, mchi , limit_obs[count]  , limit_exp[count] , limit_sm1[count] , limit_sp1[count], hxsec.GetBinContent(hxsec.FindBin(msbo)) ) )

    else:
        v_xsec.append( limit_obs[count] )

    # h_xsec_test.SetBinContent(h_xsec_test.FindBin(msbo,mchi), limit_obs[count]*hxsec.GetBinContent(hxsec.FindBin(msbo)))
    # h_sobs_test.SetBinContent(h_sobs_test.FindBin(msbo,mchi), limit_obs[count])
    count = count+1
...
g_xsec = ROOT.TGraph2D("g_xsec","",count,msbos,mchis,v_xsec)
if not doSignificance:
    g_sexp = ROOT.TGraph2D("g_sexp","",count,msbos,mchis,v_sexp)
    g_ssm1 = ROOT.TGraph2D("g_ssm1","",count,msbos,mchis,v_ssm1)
    g_ssp1 = ROOT.TGraph2D("g_ssp1","",count,msbos,mchis,v_ssp1)
    g_sobs = ROOT.TGraph2D("g_sobs","",count,msbos,mchis,v_sobs)
    g_som1 = ROOT.TGraph2D("g_som1","",count,msbos,mchis,v_som1)
    g_sop1 = ROOT.TGraph2D("g_sop1","",count,msbos,mchis,v_sop1)
...
if not doSignificance:
    ...
    l1.SetHeader("pp #rightarrow #tilde{b}_{1}#bar{#tilde{b}}_{1}, #tilde{b}_{1}#rightarrow tW#tilde{#chi}^{0}_{1}    NLO+NLL cross section")
...
c1.SaveAs("t6ttww_scan_xsec_%s%s%s.pdf" % (lumi_str,postfix, "_sig" if doSignificance else ""))
...
if not doSignificance:
    ...
    fout = ROOT.TFile("SUS15008_t6ttww_%s%s.root" % (lumi_str,postfix),"RECREATE")
...
os.system("web t6ttww_scan_xsec_%s%s%s.pdf" % (lumi_str,postfix,"_sig" if doSignificance else ""))

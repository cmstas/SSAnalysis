...
for msbo in range (minx,maxx,step):
    for mchi in range (miny,maxy,step):
        if os.path.isfile(("%s/fs_t6ttww_50_m%i_m%i_histos_hihi_%sifb.root" % (mydir,msbo,mchi,mylumi))) is False: continue
        #print "found sig = fs_t6ttww_50_m%i_m%i" % (msbo,mchi)
        sigs.append(("fs_t6ttww_50_m%i_m%i" % (msbo,mchi)))
...
for sig in sigs:
    smod = sig.split("_")[1]
    msbo = int(sig.split("_")[3][1:])
    msbos.append(msbo)
    mchi = int(sig.split("_")[4][1:])
    mchis.append(mchi)
...
c1.SaveAs("t6ttww_scan_xsec.pdf")

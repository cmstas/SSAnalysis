...
for mglu in range (minx,maxx,step):
    for mlsp in range (miny,maxy,step):
        if os.path.isfile(("%s/fs_t5tttt_dm175_m%i_m%i_histos_hihi_%sifb.root" % (mydir,mglu,mlsp,mylumi))) is False: continue
        #print "found sig = fs_t5ttttdm175_m%i_m%i" % (mglu,mlsp)
        sigs.append(("fs_t5tttt_dm175_m%i_m%i" % (mglu,mlsp)))
...
# #hack
# h_xsec.SetBinContent(39,51,h_xsec.GetBinContent(39,50))
# h_xsec.SetBinContent(40,51,h_xsec.GetBinContent(40,50))
# h_xsec.SetBinContent(41,51,h_xsec.GetBinContent(41,50))
# h_xsec.SetBinContent(42,51,h_xsec.GetBinContent(42,50))
# h_xsec.SetBinContent(43,51,h_xsec.GetBinContent(43,50))
# h_xsec.SetBinContent(44,51,h_xsec.GetBinContent(44,50))
# h_xsec.SetBinContent(45,51,h_xsec.GetBinContent(45,50))
...
diagtex = ROOT.TLatex(0.20,0.325, "m_{#tilde{g}} - m_{#tilde{#chi}^{0}_{1}} = m_{t} + m_{W} + m_{b}" )
diagtex.SetNDC()
...
if not doSignificance:
    ...
    l1.SetHeader("pp #rightarrow #tilde{g}#tilde{g}, #tilde{g}#rightarrow #tilde{t}_{1}#kern[0.3]{t}, #tilde{t}_{1}#rightarrow t#tilde{#chi}^{0}_{1}  NLO+NLL cross section")
...
c1.SaveAs("t5tttt_dm175_scan_xsec_%s%s%s.pdf" % (lumi_str,postfix, "_sig" if doSignificance else ""))
...
if not doSignificance:
    ...
    fout = ROOT.TFile("SUS15008_t5tttt_dm175_%s%s.root" % (lumi_str,postfix),"RECREATE")

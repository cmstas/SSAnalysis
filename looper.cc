...
int looper::ScanChain(TChain* chain, TString prefix, TString suffix, bool isData, TString whatTest, int nEvents, vector<int> evtToDebug){
  ...
  if (makebaby){
    ...
    bm->MakeBabyNtuple( Form( "%s%s", prefix.Data(), suffix.Data() ), isFastSim);
  }
  ...
  //Add good run list
  set_goodrun_file("goodRunList/final2015_golden_25ns2p26fb.txt");
  ...
  while ((currentFile = (TFile*)fileIter.Next())){
	...
	continue;
      ...
      int br = baselineRegion(njets, nbtag, met, ht, hypleps[0].pdgId(), hypleps[1].pdgId(), hypleps[0].pt(), hypleps[1].pt());
      if (br < 0){
        if (debug){
